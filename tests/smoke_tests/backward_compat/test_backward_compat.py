...
class TestBackwardCompatibility:
    ...
    def test_client_server_compatibility_old_server(self, generic_cloud: str):
        ...
        if self.BASE_API_VERSION < self.CURRENT_MIN_COMPATIBLE_API_VERSION or \
            ...
            if self.BASE_API_VERSION < 12:
                pytest.skip(
                    f'Base API version: {self.BASE_API_VERSION} is too old, the enforce compatibility is supported after 12(release 0.11.0)'
                )
            if self.CURRENT_API_VERSION < 12:
                pytest.skip(
                    f'Current API version: {self.CURRENT_API_VERSION} is too old, the enforce compatibility is supported after 12(release 0.11.0)'
                )
