...
# Re-export for backward compatibility (websocket_proxy.py imports this).
SSHMessageType = ssh_proxy.SSHMessageType
...
app.include_router(ssh_proxy.ssh_router)
...
@app.get('/ssh-proxy-info')
async def ssh_proxy_info(cluster_name: str,
                         endpoint: str = 'kubernetes-pod-ssh-proxy'):
    """Check if SSH should be redirected to an agent.

    Returns redirect info or {redirect: false}.
    Plugins can override this endpoint to provide agent-aware redirects.
    """
